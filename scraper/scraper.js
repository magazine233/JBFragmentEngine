...
class MyGovScraper {
  ...
  async crawl(browser, url, robots, depth = 0, retries = 3) {
    ...
    try {
      ...
      await page
        .waitForSelector("main, #main-content, .main-content, article", {
          timeout: 5000,
        })
        .catch(() => {});
        ...
        const priorityLinks = links
          .filter((l) => {
            try {
              ...
              return (
                linkUrl.origin === new URL(location.href).origin &&
                !l.href.match(/\.(pdf|doc|docx|xls|xlsx)$/i) &&
                !l.href.includes("#") &&
                !this.cfg.excludePatterns.some((pattern) =>
                  pattern.test(l.href),
                )
              );
            } catch {
  ...
  async extractFragments($, url, page) {
    ...
    try {
      ...
      for (let i = 0; i < headings.length; i++) {
        ...
        if (
          $content.length === 0 &&
          $heading.parent().is("div, section, article")
        ) {
        ...
        if ($content.length > 0 || $heading.is("h1")) {
          try {
            ...
            console.error(
              `Error processing fragment in ${url}:`,
        ...
        try {
          ...
          for (const elem of elements) {
            ...
            try {
              ...
              for (const hSelector of headingSelectors) {
                ...
                for (let h = 0; h < $headings.length; h++) {
                  ...
                  if (
                    $h.get(0) &&
                    $elem.get(0) &&
                    $h.get(0).compareDocumentPosition($elem.get(0)) &
                      Node.DOCUMENT_POSITION_FOLLOWING
                  ) {
                    // Check if there's another heading between them
                    const nextHeading = $h
                      .nextAll(headingSelectors.join(","))
                      .first();
                    if (
                      !nextHeading.length ||
                      nextHeading.get(0).compareDocumentPosition($elem.get(0)) &
              ...
              console.log(
                `Skipping hierarchy check for standalone element: ${closestError.message}`,
              );
            }

            if (!isPartOfHeading) {
              try {
                const fragment = this.extractStandaloneFragment(
                  $elem,
                  url,
                  breadcrumbs,
                );
                if (fragment) {
                  ...
                  fragments.push({
                    ...
                    last_seen_at: Date.now(),
                    popularity_sort:
                      100 - (enrichedFragment.popularity_score || 0),
                  });
                }
              } catch (standaloneError) {
                console.error(
                  `Error processing standalone element in ${url}:`,
          ...
          console.error(
            `Error with selector ${selector} in ${url}:`,
  ...
  extractBreadcrumbs($) {
    ...
    for (const selector of selectors) {
      try {
        ...
        console.log(
          `Error extracting breadcrumbs with selector ${selector}:`,
          breadcrumbError.message,
        );
      }
    }
  ...
  extractStandaloneFragment($elem, url, breadcrumbs) {
    ...
    return {
      ...
      url: `${url}#${id}`,
      title:
        $elem.find("h1, h2, h3, h4").first().text() ||
  ...
  detectComponentType($elem) {
    ...
    if ($elem.hasClass("checklist") || $elem.find("ol.steps").length)
      return "checklist";
    if ($elem.hasClass("alert") || $elem.is('[role="alert"]')) return "alert";
    if ($elem.hasClass("info-box") || $elem.hasClass("info-panel"))
      return "info_box";
    return "content";
  }
  ...
  async indexFragments() {
    ...
    for (let i = 0; i < this.fragments.length; i += 100) {
      ...
      try {
        ...
        console.log(
          `Progress: ${indexed}/${this.fragments.length} (${Math.round((indexed / this.fragments.length) * 100)}%)`,
        );
      } catch (e) {
        ...
        for (const doc of batch) {
          try {
            ...
            console.error(
              `Failed to index fragment: ${doc.url}`,
              docErr.message,
            );
          }
        }
